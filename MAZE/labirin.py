...
import random
from generic_search import Stack, Queue, node_to_path, Node
from tkinter import *
...
class MazeGUI:
    ...
    def _setup_GUI(self):
        ...
        style.configure("BG.TLabel", foreground="black",
                        font=('Helvetica', 26))
        style.configure("BG.TButton", foreground="black",
                        font=('Helvetica', 26))
        style.configure("BG.TListbox", foreground="black",
                        font=('Helvetica', 26))
        style.configure("BG.TCombobox", foreground="black",
                        font=('Helvetica', 26))
        style.configure(" ", foreground="black", background="white")
        style.configure(Cell.EMPTY.value + ".TLabel", foreground="black",
                        background="white", font=('Helvetica', 26))
        style.configure(Cell.BLOCKED.value + ".TLabel", foreground="white",
                        background="black", font=('Helvetica', 26))
        style.configure(Cell.START.value + ".TLabel", foreground="black",
                        background="red", font=('Helvetica', 26))
        style.configure(Cell.GOAL.value + ".TLabel", foreground="black",
                        background="yellow", font=('Helvetica', 26))
        style.configure(Cell.PATH.value + ".TLabel", foreground="black",
                        background="red", font=('Helvetica', 26))
        style.configure(Cell.EXPLORED.value + ".TLabel", foreground="black", background="orange",
                        font=('Helvetica', 26))
        style.configure(Cell.CURRENT.value + ".TLabel", foreground="black",
                        background="red", font=('Helvetica', 26))
        style.configure(Cell.FRONTIER.value + ".TLabel", foreground="black", background="white",
                        font=('Helvetica', 26))

        # put labels on the side
        for row in range(self._rows):
            Grid.rowconfigure(frame, row, weight=1)
            row_label: Label = Label(frame, text=str(
                row), style="BG.TLabel", anchor="center")
        ...
        for column in range(self._columns):
            ...
            column_label: Label = Label(frame, text=str(
                column), style="BG.TLabel", anchor="center")
            column_label.grid(row=self._rows, column=column +
                              1, sticky=N + S + E + W)
        ...
        self._display_grid()
        # setup buttons
        dfs_button: Button = Button(
            frame, style="BG.TButton", text="Run DFS", command=self.dfs)
        dfs_button.grid(row=self._rows + 2, column=0,
                        columnspan=6, sticky=N + S + E + W)
        bfs_button: Button = Button(
            frame, style="BG.TButton", text="Run BFS", command=self.bfs)
        bfs_button.grid(row=self._rows + 2, column=6,
                        columnspan=6, sticky=N + S + E + W)
        Grid.rowconfigure(frame, self._rows + 2, weight=1)
        # setup data structure displays
        frontier_label: Label = Label(
            frame, text="Frontier", style="BG.TLabel", anchor="center")
        frontier_label.grid(row=0, column=self._columns + 2,
                            columnspan=3, sticky=N + S + E + W)
        explored_label: Label = Label(
            frame, text="Explored", style="BG.TLabel", anchor="center")
        explored_label.grid(
            row=self._rows // 2, column=self._columns + 2, columnspan=3, sticky=N + S + E + W)
        ...
        self._frontier_listbox: Listbox = Listbox(
        ...
        self._explored_listbox: Listbox = Listbox(
            frame, font=("Helvetica", 14))
        ...
        interval_label: Label = Label(
            frame, text="Interval", style="BG.TLabel", anchor="center")
        interval_label.grid(
            row=self._rows + 1, column=self._columns + 2, columnspan=3, sticky=N + S + E + W)
        self._interval_box: Combobox = Combobox(frame, state="readonly", values=[1, 2, 3, 4, 5], justify="center",
                                                style="BG.TCombobox")
        self._interval_box.set(2)
        self._interval_box.grid(
            row=self._rows + 2, column=self._columns + 2, columnspan=3, sticky=N + S + E + W)
        # pack and go
    ...
    def step(self, frontier, explored, last_node):
        if not frontier.empty:
            ...
            self.root.after(num_delay, self.step, frontier,
